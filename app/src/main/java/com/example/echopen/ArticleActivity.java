...
public class ArticleActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (currentUserId != null) {
            blogAdapter = new ArticleAdapter(this, new ArrayList<>(), new ArticleAdapter.OnItemClickListener() {
                @Override
                public void onEditClick(BlogItemModel blogItem) {
                    Intent intent = new Intent(ArticleActivity.this, EditBlogActivity.class); // Changed to EditBlogActivity
                    intent.putExtra("blogItem", blogItem); // Pass the blog item with postId
                    startActivityForResult(intent, EDIT_BLOG_REQUEST_CODE);
                }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        super.onActivityResult(requestCode, resultCode, data);
        if (requestCode == EDIT_BLOG_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
            if (data != null && data.hasExtra("updatedBlogItem")) {
                BlogItemModel updatedBlogItem = data.getParcelableExtra("updatedBlogItem");
                updateBlogItemInView(updatedBlogItem);
            }
        }
    }

    private void updateBlogItemInView(BlogItemModel updatedBlogItem) {
        List<BlogItemModel> blogList = blogAdapter.getBlogList();
        for (int i = 0; i < blogList.size(); i++) {
            if (blogList.get(i).getPostId().equals(updatedBlogItem.getPostId())) {
                blogList.set(i, updatedBlogItem);
